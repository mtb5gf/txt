GITHUB https://github.com/mtb5gf/txt
import re
...
def placer(lst1, lst2):
	...
	for item in lst1:
		food = item[1]
		x = tokenizer.findall(food)
		for word in x:
			ingredient = word
			for step in range(len(lst2)):
				if ingredient in lst2[step]:
...
def food_grouper (lst):
	...
	Egg = ['egg', 'egg white', 'yolk']
	Grain = ['flour']
	Staple = ['baking soda', 'sugar', 'brown sugar', 'baking powder', 'salt', 'chocolate', 'vegetable oil', 'vanilla extract']
	Grouped = []
	for i in range(len(lst)):
		if lst[i][1] in Dairy:
			classification = 'Dairy'
			Grouped.append((lst[i][0], lst[i][1], lst[i][2], classification))
		elif lst[i][1] in Egg:
			classification = 'Egg'
			Grouped.append((lst[i][0], lst[i][1], lst[i][2], classification))
		elif lst[i][1] in Grain:
			classification = 'Grain'
			Grouped.append((lst[i][0], lst[i][1], lst[i][2], classification))
		elif lst[i][1] in Staple:
...
###Make a function that will find all of the tools and pans needed in a recipe###
def PANderer (lst):
